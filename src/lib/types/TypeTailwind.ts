// Definisikan breakpoints Tailwind
type BreakpointPrefix = 'sm:' | 'md:' | 'lg:' | 'xl:' | '2xl:';

type Responsive<T extends string> = T | `${BreakpointPrefix}${T}`;

export type TailwindSizing = Responsive<
 | 'w-max' | 'w-full' | 'w-screen' | 'w-auto' | 'w-8' | 'w-16' | 'w-32' | 'w-64' | 'w-1/2' | 'w-1/3' | 'w-1/4' | 'w-3/4' | 'w-4/5' | 'w-2/3'
 | 'h-max' | 'h-full' | 'h-screen' | 'h-auto' | 'h-8' | 'h-16' | 'h-32' | 'h-64' | 'h-1/2' | 'h-1/3' | 'h-1/4' | 'h-3/4' | 'h-4/5' | 'h-[700px]'
>;
...
export type TailwindFlexbox = Responsive<
  ...
  | 'gap-0' | 'gap-4' | 'gap-6' | 'gap-8' | 'gap-12' | 'gap-16'
>;
...
export interface LayoutProps extends TailwindResponsiveProps, React.HTMLAttributes<HTMLElement> {
  children: React.ReactNode;
  bg?: | 'bg-white' | 'bg-black' | 'bg-light' | 'bg-dark' | 'bg-darkSecondary' | 'bg-lightSecondary'
